...
def create(n):
    # s is the board itself. a vector that represent a matrix. s=[0,1,2....n^2-1]
    s = list(range(n*n))
    # m is "<>v^" - for every possible move (left, right, down, up)
    m = "<>v^"
    for i in range(n*n*n):  # makes n^3 random moves
        if_legal(s, m[random.randrange(4)])
    # at the beginning "" is an empty path, later on path
    return [s, ""]
    ...
    for i in "<>v^":
        ...
        if s.index(0) != x[0].index(0) and \
           (x[1] == "" or x[1][-1] != "><^v"["<>v^".index(i)]):  # check if it's the first move or it's a reverse move
            list_of_steps = x[1]+i
            # appends the new state to ns
            ns.append([s, list_of_steps, len(list_of_steps)])
    return ns
...
def hdistance(s):                   # the heuristic value of s
    # return with no heuristics
    return 0

    # return with tiles out of place heuristics
"""
    counter = 0
    board = s[0]
    ...
    # manhattan
    size_n = math.sqrt(len(s[0]))
    initial_state = s[0]
    initial_config = initial_state
    manDict = 0
    for i, item in enumerate(initial_config):
        prev_row, prev_col = int(i / size_n), i % size_n
        goal_row, goal_col = int(item / size_n), item % size_n
